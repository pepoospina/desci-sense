export const DEBUG = true;

export const FUNCTIONS_BASE = process.env.FUNCTIONS_BASE;

export const ORCID_CLIENT_ID = process.env.ORCID_CLIENT_ID;
export const ORCID_API_URL = 'https://orcid.org';
export const ORCID_REDIRECT_URL = 'http://127.0.0.1:3000/';

export const TWITTER_API_URL_AUTH = 'https://api.twitter.com/1.1';
export const TWITTER_API_URL = 'https://api.twitter.com/2';
export const TWITTER_CLIENT_ID = process.env.TWITTER_CLIENT_ID;
export const TWITTER_CALLBACK_URL_ENCODED = 'http%3A%2F%2F127.0.0.1%3A3000%2F';
